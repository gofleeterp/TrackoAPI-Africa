...
using util = TrackoAPI.WebUtilities.Formatters.WebApiContrib.Formatting.Xlsx.FormatterUtils;
...
namespace TrackoAPI.WebUtilities.Formatters.WebApiContrib.Formatting.Xlsx
{
    ...
    public class XlsxMediaTypeFormatter : MediaTypeFormatter
    {
        ...
        public override Task WriteToStreamAsync(Type type1, object value, System.IO.Stream writeStream, System.Net.Http.HttpContent content, System.Net.TransportContext transportContext)
        {
            // Create a worksheet
            ...
            var isDataTable = valueType == typeof(DataTable);
            if (isDataTable)
            {
                ...
                var dtcells = worksheet.Cells[worksheet.Dimension.Address];

                // Add autofilter and fit to max column width (if requested).
                if (AutoFilter) dtcells.AutoFilter = AutoFilter;
                if (AutoFit) dtcells.AutoFitColumns();
