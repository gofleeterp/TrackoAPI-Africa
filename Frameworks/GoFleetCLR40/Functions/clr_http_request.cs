...
{
    [SqlFunction]
    public static System.Data.SqlTypes.SqlString UrlEncode(SqlString text)
    {
        return new SqlString(WebUtility.UrlEncode(text.Value));
    }
    [SqlFunction]
    public static System.Data.SqlTypes.SqlString UrlDecode(SqlString text)
    {
        return new SqlString(WebUtility.UrlDecode(text.Value));
    }
    [SqlFunction]
    ...
    {
        string method = requestMethod.IsNull ? "GET" : requestMethod.Value;
        string urlStr = url.IsNull ? "" : url.Value;
        string paramStr = parameters.IsNull ? "" : parameters.Value;
        string headerStr = headers.IsNull ? "" : headers.Value;


        // If GET request, and there are parameters, build into url
        if (method.ToUpper() == "GET" && !string.IsNullOrWhiteSpace(paramStr))
            urlStr += (urlStr.IndexOf('?') > 0 ? "&" : "?") + paramStr;
        ...
        if (!string.IsNullOrWhiteSpace(headerStr))
        {
            ...
            var xmlData = XElement.Parse(headerStr);
        ...
        try
        {
            ...
            if (method.ToUpper() != "GET" && !string.IsNullOrWhiteSpace(paramStr))
            {
                ...
                var parameterData = Encoding.ASCII.GetBytes(paramStr);
        ...
        catch (Exception ex)
        {
            returnXml =
               new XElement("Response",
                   new XElement("Body", ex.GetBaseException().ToString())
               );
        }
