namespace CronExpressionDescriptor
{
    ...
    public static class MyCron
    {
        /// <summary>
        /// Gets the Next DateTime List from Cron Expression
        /// </summary>
        /// <param name="cron"></param>
        public static List<DateTime> GetNextExecutions(DateTime startDate,DateTime? endDate,string cron)
        {
            var exp = CrontabSchedule.Parse(cron);
           return exp.GetNextOccurrences(startDate, endDate ?? startDate.AddYears(1)).ToList();
        }
        /// <summary>
        /// Gets the Next DateTime List from Cron Expression
        /// </summary>
        /// <param name="cron"></param>
        public static DateTime GetNextExecution(DateTime startDate, DateTime? endDate, string cron)
        {
            var exp = CrontabSchedule.Parse(cron);
            return exp.GetNextOccurrence(startDate, endDate ?? startDate.AddYears(1));
        }
        /// <summary>
        ...
        public static string GetDescription(string cronExpression, bool throughException = false)
        {
            return ExpressionDescriptor.GetDescription(
                cronExpression,
                new Options()
                {
                    ThrowExceptionOnParseError = throughException,
                    Use24HourTimeFormat=true,
                    Locale="en-IN"
                });
        }
        public static string GetNCrontabDescription(string cron)
        {
            var exp = CrontabSchedule.Parse(cron);
            return exp.ToString();
        }
        public static string GetDescription(CronViewModel cronModel,bool throughException=false)
        {
            if(cronModel==null) throw new InvalidCastException("Cron Expression cannot be null or  empty");
            string[] expressionParts = cronModel.ToString().Split(' ');
            ...
            if (!throughException)
            {
                return CronExpressionDescriptor.ExpressionDescriptor.GetDescription(cronModel.ToString());
            }
